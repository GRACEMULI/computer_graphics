...
def generate_excel_files(data, output_dir, pivot_language='de'):
...
def main():
    ...
    if generate_language_excel.lower() == 'y':
        ...
        generate_excel_files(data, output_dir, pivot_language='de')  # You can specify the pivot language here.
    else:
        # Generate Excel files for all languages
        for language_code, language_data in data.items():
            df = pd.DataFrame(language_data)
            output_filename = os.path.join(output_dir, f"de-{language_code}.xlsx")
